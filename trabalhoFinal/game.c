...
int moveEnemyToBase(int enemy[3][10], int base[3], int j, int b){ //funcao que move o inimigo em direcao a base
    ...
    if(enemy[1][j] - base[1] >= 0 && b % 32 == 0){ //se o inimigo está a direita da base
        enemy[1][j] = enemy[1][j] - 1;
    }
    if(enemy[1][j] - base[1] < 0 && b % 32 == 0){ //se o inimigo está a esquerda da base
        enemy[1][j] = enemy[1][j] + 1;
    }
    if(enemy[2][j] - base[2] >= 0 && b % 32 !=0){ //se o inimigo está acima da base
        enemy[2][j] = enemy[2][j] - 1;
    }
    if(enemy[2][j] - base[2] < 0 && b % 32 !=0){ //se o inimigo está abaixo da base
        enemy[2][j] = enemy[2][j] + 1;
